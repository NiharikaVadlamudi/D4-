...
'''
Normalise the dataframe -- using standardScaler 
'''
def normaliseColumns(df):
    taskFilter = df['taskMask']
    taskFilter = [ converttoList(task) for task in taskFilter ] 
    cols = df.columns.values.tolist()
    y_cols = [col for col in cols if col.find('y_')>=0]
    ntasks = len(y_cols)
    print('nTASKS : {}'.format(ntasks))
    for tindex,taskName in enumerate(y_cols):
        scaler = StandardScaler()
        print('Normalising the task -- {}'.format(taskName))
        coldata = df[str(taskName)]
        orgLen = len(coldata)
        indices = [ i for i,task in enumerate(taskFilter) if task[tindex]==1]
        refined_data = [ele for j,ele in enumerate(coldata) if taskFilter[j][tindex]==1]
        print('REFLDATA : {}'.format(len(refined_data)))
        scaled_data =  scaler.fit_transform(np.asarray(refined_data).reshape(-1,1))
        scaled_data =scaled_data.reshape(-1,)
        scaled_data=scaled_data.tolist()
        # Change the dataframe 
        new_data = [0.0]*(orgLen)
        for i,index in enumerate(indices):
            new_data[index]=scaled_data[i]
        df[taskName+'_new']=new_data
        print('New column created ...{}'.format(taskName+'_new'))
...
class Trainer(object):
    ...
    def __init__(self, args,norm=False):
        self.args=args 
        ...
        train_df = pd.read_csv('./data/illa_corr_reg_train_descriptors.csv')
        valid_df = pd.read_csv('./data/illa_corr_reg_test_descriptors.csv')
        
        # Apply Normalisation 
        if(norm):
            self.train_df=normaliseColumns(train_df)
            self.valid_df=normaliseColumns(valid_df)
        else:
    ...
    def loop(self):
        for epoch in range(self.epoch,self.maxEpochs):
            ...
            weights_=self.train(self.epoch)
    ...
    def train(self,epoch,wgt_train_loader=None):
        ...
        for step,samples in enumerate(self.data_loader):
            try:
                ...
                labels=torch.tensor(samples[2]).to(device).float()
                y_out= self.network([drug_graphs])
                y_out=y_out.reshape((self.batchSize,self.ntasks,1))
                ...
                running_mse_loss.append(illa_loss.cpu().detach().numpy())
                running_mae_loss.append(loss.cpu().detach().numpy())

                # print('LOSS for mini-batch is : {}'.format(illa_loss.cpu().detach().numpy()))
