    ...
    def weightGeneration(self):
        ...
        for step,samples in enumerate(self.weight_train_loader):
            ...
            try:
                ...
                y_out=y_out.reshape((self.samplebatchSize,1,self.ntasks))

                # Loss Computation - MAE alone 
                mae_reg_loss = self.mae_loss_fn(y_out,labels).reshape((self.samplebatchSize,self.ntasks,1))
                loss_= torch.bmm(taskMasks,mae_reg_loss)
                loss_= torch.mean(loss_,dim=0)
                epochWeights[step]=loss_.cpu().detach().numpy().item()
                # print('Epoch : {} Sample Point : {} Weight : {}'.format(self.epoch,step,epochWeights[step]))
            except Exception as exp : 
                print('Error : {}'.format(exp))
                epochWeights[step]=1.0
                skipped+=1
                continue
    ...
    def train(self,epoch,wgt_train_loader=None):
        ...
        for step,samples in enumerate(self.data_loader):
            try:
                ...
                running_mae_loss.append(illa_loss.cpu().detach().numpy())
